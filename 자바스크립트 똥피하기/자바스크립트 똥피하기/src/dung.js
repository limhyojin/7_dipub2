...
function Dung(_game) {
	...
	this.drop = function() {
		...
		// 바닥과 거의 가까워졌으면 (충돌검사)
		if (pos[1] >= screen_height - 112 && game.playing) {
			if (game.playing && game.man.collision(pos[0] + 22 / 2)) // 충돌했으면
				game.gameOver();
		}
	};
...
function Man(_game) {
	...
	this.initialize = function(_game) {
		...
		img = new ImageList("images/man.gif", 77, 112);
		
		img.object.style.position = "absolute";
		img.object.style.top = "-112px";
		
		game.screen.appendChild(img.object);
	};
	
	this.collision = function(dung_x) {
		return (Math.abs(dung_x - (posx + 90 / 2)) < 17);
	};
	...
	this.spawn = function() {
		...
		img.object.style.top = (game.size[1] - 112) + "px";
	...
	this.move = function() {
		...
		if (!dir) { // 멈춰야 되면
			...
			img.showImage((dir == Event.KEY_LEFT ? 0 : 2) + action);
